...
lib_deform = Library("deform", os.sep.join((os.path.dirname(deform.__file__), "static")))
deform_js = Resource(lib_deform, "scripts/deform.js")
jquery_form_js = Resource(lib_deform, "scripts/jquery.form.js", depends=[jquery, ])

# deform_bootstrap doesn't provide fanstatic resources itself
# this should be moved to deform_bootstrap in the future
lib_deform_bootstrap = Library("deform_bootstrap", os.sep.join((os.path.dirname(deform_bootstrap.__file__), "static")))
deform_bootstrap_js = Resource(lib_deform_bootstrap, "deform_bootstrap.js", depends=[deform_js, ])

# Kotti's resources
lib_kotti = Library("kotti", "static")
kotti_js = Resource(lib_kotti, "kotti.js", bottom=True)
base_css = Resource(lib_kotti, "base.css", depends=[bootstrap_responsive_css, ])
edit_css = Resource(lib_kotti, "edit.css", depends=[bootstrap_responsive_css, ])
view_css = Resource(lib_kotti, "view.css", depends=[base_css, ])
...
edit_needed = NeededGroup([jquery, bootstrap_js, edit_css, tagit, kotti_js, timepicker, jquery_form_js, deform_bootstrap_js, ])
